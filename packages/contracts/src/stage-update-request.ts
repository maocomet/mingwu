 ...
 * 第一个用户决定入口为“要求 AI 补充说明”：用户只把 pending 申请标记为
 * needs_changes 并保存决定说明，不得批准申请、不得修改正式 Stage。批准、拒绝
 * 留给后续用户接口批次。
 *
 * 字段信任边界：
 ...
 *   不得提交或伪造；
 * - decision：创建时 null；本批“要求补充”成功后写入固定
 *   { type: 'needs_changes', note, decidedAt }，note 为 trim 后的决定说明，
...
/** 申请处理状态：本批创建只产生 pending；用户“要求补充”写入 needs_changes。 */
...
/** 本批已实现的用户决定类型：只支持“要求 AI 补充说明”。 */
export const STAGE_UPDATE_REQUEST_DECISION_TYPES = ['needs_changes'] as const;
export type StageUpdateRequestDecisionType = (typeof STAGE_UPDATE_REQUEST_DECISION_TYPES)[number];

/**
 * 用户决定记录。本批只产生 needs_changes：type 固定、note 为 trim 后的决定说明、
...
export interface StageUpdateRequest {
  ...
  reason: string;
  /** 创建时固定 pending；本批“要求补充”写入 needs_changes。禁止伪造决定结果。 */
  status: StageUpdateRequestStatus;
  /** 创建时 1；每次成功用户决定 +1。客户端不能提交。 */
  revision: number;
  /** 创建时等于 createdAt；每次成功决定由服务端刷新。客户端不能提交。 */
  updatedAt: string;
  /** 创建时 null；本批“要求补充”成功后包含 needs_changes 决定。客户端不能提交。 */
...
/**
 * 用户“要求 AI 补充说明”的输入。本批 App API 代表单用户用户操作（正式用户认证
 * 留后续安全批次），因此不接收决定者 Actor / decidedAt / status / revision 等
 * 受保护字段。
 */
...
/**
 * 要求补充请求体严格白名单：只允许 expectedRevision + note。
